...
export async function handleAPIRequests(method, path, req, res, currentConfig, apiService, poolManager, promptLogFilename) {
    ...
    if (method === 'POST') {
        ...
        if (path === '/cc/v1/messages') {
            await handleContentGenerationRequest(req, res, apiService, ENDPOINT_TYPE.CLAUDE_CODE_MESSAGE, currentConfig, promptLogFilename, poolManager, currentConfig.uuid);
            return true;
        }
    }
