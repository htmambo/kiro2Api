...
export class Router {
     ...
     * @example
     * // 路由: /^\/api\/accounts\/([^\/]+)$/
     * // 路径: /api/accounts/abc-123
     * // match = ['/api/accounts/abc-123', 'abc-123']
     * // 返回: { param1: 'abc-123' }
     */
    extractParams(match) {
        const params = {};
 ...
 *
 * // 添加静态路由
 * router.addRoute('GET', '/api/health', async ({ res }) => {
 *     res.writeHead(200, { 'Content-Type': 'application/json' });
 *     res.end(JSON.stringify({ status: 'ok' }));
 * }, { auth: false, description: '健康检查' });
 *
 * // 添加正则路由
 * router.addRoute('DELETE', /^\/api\/accounts\/([^\/]+)$/, async ({ res, match }) => {
 *     const uuid = match[1]; // 提取路径参数
 *     // ... 业务逻辑
 * }, { auth: true, description: '删除账号' });
 *
 * // 匹配路由
 * const matched = router.match('GET', '/api/health');
 * if (matched) {
 *     await matched.route.handler({ res });
 * }
 *
 * // 生成文档
 * const doc = router.generateMarkdownDoc();
 * console.log(doc);
 */
